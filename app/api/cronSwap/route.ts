...
const getTargetPools = (): string[] => {
    const envPools = process.env.POOL_IDS?.split(",").map(s => s.trim()).filter(Boolean) || []
...
export async function GET() {
    try {
        if (!process.env.BOT_PRIVATE_KEY) {
            return NextResponse.json({ error: "BOT_PRIVATE_KEY not configured" }, { status: 400 })
        }
        ...
        if (solBalance < 0.05) {
            return NextResponse.json({ error: "Bot SOL balance too low", solBalance }, { status: 400 })
        }
        ...
        for (const poolId of TARGET_POOLS) {
            try {
                ...
                if (!poolInfoList || poolInfoList.length === 0 || !poolInfoList[0]) {
                    ...
                    results.push({ poolId, status: "skipped", reason: "not indexed by Raydium API yet" })
                    continue
                }

                const poolInfo = poolInfoList[0] as ApiV3PoolInfoConcentratedItem
                console.log(`📊 ${poolInfo.mintA.symbol}/${poolInfo.mintB.symbol} | price=${poolInfo.price}`)

                // Random direction to keep pool balanced over time
                const swapAtoB = Math.random() > 0.5
                const inputMintAddress = swapAtoB ? poolInfo.mintA.address : poolInfo.mintB.address
                const inputDecimals = swapAtoB ? poolInfo.mintA.decimals : poolInfo.mintB.decimals
                const outputDecimals = swapAtoB ? poolInfo.mintB.decimals : poolInfo.mintA.decimals
                ...
                console.log(`↔️ ${amountFloat.toFixed(3)} ${symIn} → ~${expectedOut.toFixed(3)} ${symOut}`)
