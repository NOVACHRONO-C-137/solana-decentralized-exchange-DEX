...
function WithdrawCLMMContent() {
    ...
    const loadPositions = useCallback(async () => {
        ...
        try {
            ...
            const decimalsA = info.mintA.decimals || 6;
            const decimalsB = info.mintB.decimals || 6;
    ...
    const formatTick = (tick: number, decimalsA: number, decimalsB: number) => {
        if (tick <= -443636) return "0.00";
        if (tick >= 443636) return "∞";
        ...
        if (adjustedPrice < 0.000001) {
            ...
            return adjustedPrice.toLocaleString(undefined, {
                        ...
                        <div key={pos.nftMint} className={`${glassCard} p-5 flex flex-col gap-4`}>
                            ...
                            <div className="bg-white/50 dark:bg-black/20 border border-black/[0.08] dark:border-white/[0.06] rounded-xl px-4 py-3 grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-foreground">{formatTick(pos.tickLower, pos.decimalsA, pos.decimalsB)}</p>
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-foreground">{formatTick(pos.tickUpper, pos.decimalsA, pos.decimalsB)}</p>
                                    <p className="text-[10px] text-muted-foreground">{symbolB} per {symbolA}</p>
                                </div>
                            </div>
                            ...
                            <div className="bg-white/50 dark:bg-black/20 border border-black/[0.08] dark:border-white/[0.06] rounded-xl px-4 py-3">
                                ...
                                <p className="text-lg font-bold text-foreground">
                                    {formatLargeNumber(pos.liquidityHuman)}
                                ...
                                <div className="bg-[var(--neon-teal)]/5 border border-[var(--neon-teal)]/20 rounded-xl px-4 py-3">
                                    ...
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className="font-bold text-foreground">
                                            {formatLargeNumber(pos.tokenFeeAmountA.toNumber() / Math.pow(10, pos.decimalsA))}
                                        </span>
                                    </div>
                                    <div className="flex justify-between text-sm mt-1">
                                        <span className="text-muted-foreground">{symbolB}</span>
                                        <span className="font-bold text-foreground">
                                            {formatLargeNumber(pos.tokenFeeAmountB.toNumber() / Math.pow(10, pos.decimalsB))}
                                        </span>
                                    </div>
                                </div>
                            )}
