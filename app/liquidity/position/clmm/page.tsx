...
function PositionPageInner() {
  ...
  const initialBInfo =
    DEVNET_TOKENS.find((t) => t.symbol === tokenB) || fallbackB;
    ...
    const fetchPoolData = async () => {
      ...
      try {
        const res = await fetch(
          `https://api-v3-devnet.raydium.io/pools/info/ids?ids=${poolId}`,
        );
        ...
        if (poolInfo) {
          ...
          const needsFetchTokens =
            !initialAInfo.logoURI ||
            !initialBInfo.logoURI ||
            !initialAInfo.mint ||
            !initialBInfo.mint;
          if (needsFetchTokens) {
            const isAMintA =
              initialAInfo.symbol === poolInfo.mintA?.symbol ||
              (!initialAInfo.mint && !initialBInfo.mint);
            if (poolInfo.mintA) {
  ...
  if (poolPrice && parseFloat(minPrice) < parseFloat(maxPrice)) {
    const { ratioA: rA, ratioB: rB } = calculateCLMMRatio(
      poolPrice,
      parseFloat(minPrice),
      parseFloat(maxPrice),
    );
    ...
    try {
      ...
      if (!poolInfo) {
        const configRes = await fetch(
          "https://api-v3-devnet.raydium.io/main/clmm-config",
        );
        const configJson = await configRes.json();
        const feeRateNum = Math.round(parseFloat(fee.replace("%", "")) * 10000);
        const matchedConfig = configJson.data.find(
          (c: any) => c.tradeFeeRate === feeRateNum,
        );
      ...
      if (
        ...
        minPriceNum < currentPriceNum &&
        maxPriceNum > currentPriceNum
      ) {
        // Convert prices to ticks: tick = log(price) / log(1.0001)
        const rawTickLower = Math.floor(
          Math.log(minPriceNum) / Math.log(1.0001),
        );
        const rawTickUpper = Math.ceil(
          Math.log(maxPriceNum) / Math.log(1.0001),
        );
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-[#161722] border border-white/10 rounded-2xl px-6 py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs bg-[var(--neon-teal)]/10 text-[var(--neon-teal)] border border-[var(--neon-teal)]/20 px-2 py-0.5 rounded-full">
              {fee}
            </span>
          </div>
          <div className="flex items-center gap-4 text-sm">
            ...
            {!poolId && (
              <span className="text-yellow-400/80 text-xs">
                ⚠ Mock pool — deposit is for real pools only
              </span>
            )}
          </div>
        </div>
